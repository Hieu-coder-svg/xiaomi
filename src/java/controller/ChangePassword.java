...
public class ChangePassword extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ControlAccount</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ControlAccount at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (u == null) {
            ...
            if (!pass.equalsIgnoreCase(repass)) {
                String mess1 = "new pass and repeat pass are not same! Please enter again...";
                request.setAttribute("mess1", mess1);
                request.getRequestDispatcher("detailaccount.jsp").forward(request, response);
            }
            else{
                dao.change(pass, user);
                String mess2 = "Change password successfully!";
