...
public class ViewByChooseCategory extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException { // lấy và hiển thị sản phẩm theo danh mục được chọn
        //processRequest(request, response);
         //Select Product by CategoryID
        String id = request.getParameter("cid");
        int id1 = Integer.parseInt(id);
        HttpSession session = request.getSession();
        session.setAttribute("cid", id1);
        ...
        int count = dao.getTotalProducts(id1);
        int page = count / 15; //mỗi trang lấy 15 sản phẩm
        if (count % 15 != 0) {
            page++;
        }
